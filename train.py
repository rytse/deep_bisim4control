...
def parse_args():
    ...
    # train
    parser.add_argument(
        "--agent", default="bisim", type=str, choices=["baseline", "bisim", "deepmdp"]
    )
    ...
    parser.add_argument(
        "--k", default=3, type=int, help="number of steps for inverse model"
    )
    parser.add_argument(
        "--bisim_coef", default=0.5, type=float, help="coefficient for bisim terms"
    )
    ...
    parser.add_argument(
        ...
        default="pixel",
        type=str,
        choices=["pixel", "pixelLstm", "pixelCarla096", "pixelCarla098", "identity"],
    )
...
def evaluate(
        ...
        while not done:
            ...
            if embed_viz_dir:
                ...
                with torch.no_grad():
                    ...
                    embeddings.append(
                        agent.critic.encoder(torch.Tensor(obs).unsqueeze(0).to(device))
                        .cpu()
                        .detach()
                        .numpy()
                    )
    ...
    if embed_viz_dir:
        ...
        torch.save(
            dataset, os.path.join(embed_viz_dir, "train_dataset_{}.pt".format(step))
        )
